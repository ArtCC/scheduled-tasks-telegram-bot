...
class Task:
    ...
    Attributes:
        ...
        prompt: The text prompt to send to OpenAI (or plain text for reminders).
        ...
        is_reminder: If True, sends plain text without calling OpenAI.
    """
    ...
    def display_name(self) -> str:
        ...
        Returns:
            The custom name if set, otherwise 'Task #{id}', 'Reminder #{id}' or 'Task'.
        """
        if self.name:
            return self.name
        prefix = "Reminder" if self.is_reminder else "Task"
        return f"{prefix} #{self.id}" if self.id else prefix
